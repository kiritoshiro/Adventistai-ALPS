  ...
  <form action="<?php echo esc_url(home_url('/')); ?>" role="search" method="get">
    <div class="l-grid l-grid--7-col u-shift--left--1-col--at-large u-spacing--until-medium">
      <div class="l-grid-item l-grid-item--m--3-col">
        <input type="search" value="{{ the_search_query() }}" name="s" class="u-font--secondary--s u-theme--color--darker o-input__search" placeholder="Ieškoti..." />
      </div>
      <div class="l-grid-item l-grid-item--m--3-col">
        <div class="u-flex">
          <button type="submit" class="c-filter__button o-button u-space--right"><span class="u-icon u-icon--xs u-path-fill--white">@include('patterns.00-atoms.icons.icon-search')</span>Ieškoti</button> <!-- /.search-form__submit -->
      ...
      <div class="c-filter__form u-padding--top">
        <div class="l-grid l-grid--7-col u-shift--left--1-col--at-large">
          <div class="c-filter__form-item u-spacing--half l-grid-item">
            <h3 class="u-font--secondary--s u-font-weight--bold u-text-transform--upper u-color--gray can-be--lighter">Nustatymai</h3>
            {!! do_shortcode('[searchandfilter taxonomies="category" types="radio" class="c-filter__form-group" submit_label="Ieškoti dar kartą" hide_empty="0"]') !!}
          </div>
        </div>
      </div>
    </div><!-- ./c-filter -->
      ...
      <div class="c-search-results u-spacing--double text">
        @if (have_posts())
          ...
          @if (shortcode_exists('ajax_load_more'))
            {!! do_shortcode('[ajax_load_more container_type="div" css_classes="u-spacing--double" post_type="post, page" scroll="false" transition_container="false" button_label="Rodyti daugiau" posts_per_page="10" offset="10"]') !!}
          @else
