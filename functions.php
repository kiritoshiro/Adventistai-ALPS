...
}, ['helpers', 'setup', 'fields', 'filters', 'admin', 'template-helpers', 'ContentHelpers', 'Breadcrumbs', 'SeoDefaults']);
...
add_action( 'enqueue_block_assets', function() {
     if ( ! is_admin() ) {
          return; // Editor stylesheet is not needed on the public site.
     }
     $alps_editor_css = get_template_directory() . '/app/local/alps/css/wp-editor.css';
     wp_enqueue_style( 'alps_builder', get_template_directory_uri().'/app/local/alps/css/wp-editor.css', array(), is_readable( $alps_editor_css ) ? (string) filemtime( $alps_editor_css ) : '1.0', 'all' );
 } );
...
/**
 * Cache-busting version query string for local theme assets.
 * Accepts an asset URI within this theme, returns "?v=<filemtime>" or ''.
 */
function alps_asset_version( $asset_url ) {
    if ( ! is_string( $asset_url ) || $asset_url === '' ) {
        return '';
    }
    $theme_uri  = get_template_directory_uri();
    $theme_path = get_template_directory();
    if ( strpos( $asset_url, $theme_uri ) !== 0 ) {
        return '';
    }
    $relative = substr( $asset_url, strlen( $theme_uri ) );
...
/**
 * Site overrides stylesheet.
 * Enqueued from functions.php (never Blade-cached) so it loads even if the
 * compiled view cache is stale. All rules use !important, so load order
