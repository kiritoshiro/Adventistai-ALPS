@php
  ...
  // Only reserve sidebar columns when the sidebar actually has widgets in it.
  $has_sidebar = is_active_sidebar('sidebar-posts');
  ...
  if ($has_sidebar) {
    // 3-col (42.86vw) + sidebar 2-col (28.57vw) = 71.43vw = the shifted wrap.
    $article_cols   = 'l-grid-item--l--3-col';
    $article_offset = '';
  } else {
    // No sidebar: the article uses the full content width instead of
    // leaving two empty columns on the right.
    $article_cols   = 'l-grid-item--l--5-col';
    $article_offset = '';
  }
...
<section id="top" class="l-main__content l-grid l-grid--7-col {{ $section_offset }} l-grid-wrap--6-of-7 u-spacing--double--until-large">
  <div class="c-article l-grid-item {{ $article_cols }} {{ $article_offset }}">
      ...
      @if ($is_sermon)
        ...
        @if (!empty($sermon_video_embed))
          <div class="sermon-video">
            {!! wp_kses($sermon_video_embed, array('iframe'=>array('src'=>true,'width'=>true,'height'=>true,'frameborder'=>true,'allow'=>true,'allowfullscreen'=>true,'title'=>true,'loading'=>true,'referrerpolicy'=>true,'class'=>true,'style'=>true),'video'=>array('src'=>true,'controls'=>true,'width'=>true,'height'=>true,'poster'=>true,'preload'=>true,'class'=>true),'source'=>array('src'=>true,'type'=>true))) !!}
          </div>
        @endif
